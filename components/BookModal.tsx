...
export default function BookModal({ ride, lang, onClose }: Props) {
  ...
  async function confirm() {
    ...
    if (seatErr) {
      console.error("SEAT UPDATE ERR:", seatErr);
  ...
  const inp =
    "w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-800 outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100 transition placeholder-gray-400";
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="h-10 w-10 rounded-xl border border-gray-200 text-xl font-bold text-gray-600 hover:bg-violet-50 hover:border-violet-300 hover:text-violet-700 active:scale-95 transition"
                >
                  −
                </button>

                <span className="text-2xl font-black text-violet-700 w-6 text-center">
                  {seats}
                </span>
                ...
                <button
                  type="button"
                  onClick={() =>
                    setSeats((s) => Math.min(ride.seats_available, s + 1))
                  }
                  className="h-10 w-10 rounded-xl border border-gray-200 text-xl font-bold text-gray-600 hover:bg-violet-50 hover:border-violet-300 hover:text-violet-700 active:scale-95 transition"
                >
                  +
