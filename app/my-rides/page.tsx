...
import {
  ...
  IconPencil, IconTrash, IconPhone, IconUsers,
...
type Ride = {
  id: string; from_city: string; to_city: string; departure_time: string;
  vehicle_type: string; seats_available: number; price_per_seat: number;
  bookings: { id: string; passenger_name: string; phone: string; seats_booked: number }[];
};
...
export default function MyRides() {
  ...
  async function loadRides() {
    ...
    const { data, error } = await supabase
      .from("rides")
      .select(`id, from_city, to_city, departure_time, vehicle_type, seats_available, price_per_seat,
        bookings ( id, passenger_name, phone, seats_booked )`)
  ...
  if (loading) return (
    <div className="p-10 text-center text-sm font-black text-gray-500">Loading your rides...</div>
  );
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        ...
        <div className="mb-10">
          ...
          <p className="font-extrabold text-gray-600 mt-2">{t.subtitle}</p>
        </div>
        ...
        {rides.length === 0 && (
          <div className="rounded-3xl border border-dashed border-gray-200 bg-white py-20 text-center shadow-sm">
            ...
            <p className="text-lg font-black text-gray-800">No rides posted yet.</p>
          </div>
        )}
            ...
            <div key={ride.id} className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="p-6 flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-2 text-sm">
                  ...
                  <p className="font-extrabold text-gray-800 flex items-center gap-2">
                    <IconClock size={15} stroke={2} className="text-violet-400" /> {new Date(ride.departure_time).toLocaleString()}
                  </p>
                  <p className="font-extrabold text-gray-800 flex items-center gap-2">
                    ...
                    {ride.bookings.map(b => (
                      <div key={b.id} className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs font-extrabold text-gray-500 flex items-center gap-1">
                            <IconPhone size={11} stroke={2.5} /> {b.phone}
                          </p>
                        </div>
                        <div className="text-sm font-black text-violet-700">{b.seats_booked}</div>
                      </div>
                    ))}
                  </div>
                )}
